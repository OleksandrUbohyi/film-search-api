...
function requestAPI(method, url) {
    ...
    request.addEventListener('readystatechange', function() {
        ...
        output.results.forEach(function(item) {
            ...
            let itemPoster = item.poster_path;
            let releaseDate = item.release_date || item.first_air_date;
            let img = 'https://image.tmdb.org/t/p/original/' + itemPoster;
            console.log(img);
            inner +=`<div class="col-12 col-sm-6 col-lg-4 col-xl-3  mb-2">
                        <div class="border border-warning h-100">
                            <img src="${img}">
                            <div><b>Название фильма:</b> ${itemName}</div>
                                <div><b>Дата выхода первой серии:</b> <span class="badge badge-warning">${releaseDate}</span></div>
                            </div>
                    </div>`;
